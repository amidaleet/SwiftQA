...
final class SnapshotCanvas: UIViewController {
    ...
    func capture() -> UIImage? {
        ...
        case .accessibility:
            ...
            // Первый a11y-контакт поднимает iOS-инфраструктуру a11y и прогревает её на устройстве
            // (сим после erase — 0 маркеров; после активации состояние живёт до следующего erase).
        ...
        let renderer = UIGraphicsImageRenderer(size: viewToDraw.bounds.size, format: format)

        let canvasSnapshot = renderer.image { _ in
