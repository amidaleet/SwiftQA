...
final class AccessibilitySnapshotView: SnapshotAndLegendView {
    ...
    override var legendViews: [UIView] {
        displayMarkers.compactMap(\.legendView)
    }
    ...
    func parseAccessibility(useMonochromeSnapshot: Bool) throws {
        ...
        var colorBySignature: [String: UIColor] = [:]
        var legendedSignatures: Set<String> = []
        for marker in markers {
            // Легенда — справочник «цвет → озвучка»: одна запись на уникальную озвучку, дубли
            // (в т.ч. копии light+dark от DualThemeSnapshot) разделяют цвет. Пилюли рисуются
            // на всех маркерах — дубли элементов видны по позициям.
            let signature = marker.legendSignature
            let color: UIColor
            if let existing = colorBySignature[signature] {
                color = existing
            } else {
                color = markerColors[colorBySignature.count % markerColors.count]
                colorBySignature[signature] = color
            }

            let legendView: LegendView?
            if legendedSignatures.insert(signature).inserted {
                let view = LegendView(marker: marker, color: color, showUserInputLabels: showUserInputLabels)
                addSubview(view)
                legendView = view
            } else {
                legendView = nil
    ...
    private struct DisplayMarker {
        ...
        /// `nil` для дублей: пилюля есть, но легенда — одна на группу одинаковой озвучки.
...
extension AccessibilityMarker {
    /// Ключ группировки легенды: одинаковая озвучка (id/описание/подсказка/ввод/действия,
    /// без позиции) = одна запись и общий цвет.
    fileprivate var legendSignature: String {
        [
            identifier ?? "",
            description,
            hint ?? "",
            (userInputLabels ?? []).joined(separator: "\u{1F}"),
            customActions.joined(separator: "\u{1F}"),
...
extension Bundle {
    ...
    fileprivate nonisolated(unsafe) static var accessibilitySnapshotResources: Bundle = {
        (try? Bundle.snapshotsKitAssets()) ?? Bundle(for: Sentinel.self)
    }()
}
