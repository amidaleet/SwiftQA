...
extension UIImage {
    func makePNGData() throws -> Data {
        try pngData().get(elseThrow: SnapshotError("UIImage не вернул pngData()"))
    }

    func makeCGImage() throws -> CGImage {
        try cgImage.get(elseThrow: SnapshotError.cannotCreateCGImage)
    }
}

extension CGImage {
    static let snapshotColorSpace = CGColorSpaceCreateDeviceRGB()
