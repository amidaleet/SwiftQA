...
@SnapshotSuite(deviceGroup: (.phone + .tablet).universal)
struct SafeAreaSnapshotTests {
    func test_SafeArea_InView(device: SnapshotDevice) -> UIView {
        let sut = SafeAreaView()
        sut.specText = device.specText
        return sut
    }

    func test_SafeArea_InViewController(device: SnapshotDevice) -> UIView {
        let sut = SafeAreaView()
        sut.specText = device.specText
        return SafeAreaViewController(safeAreaView: sut).view
    }

    @SnapshotTest(deviceGroup: (.phone + .tablet).universal.noSafeArea)
    func test_WithoutSafeArea_InView(device: SnapshotDevice) -> UIView {
        let sut = SafeAreaView()
        sut.specText = device.specText
        return sut
    }
    ...
    @SnapshotTest(deviceGroup: (.phone + .tablet).universal.noSafeArea)
    func test_WithoutSafeArea_InViewController(device: SnapshotDevice) -> UIView {
        let sut = SafeAreaView()
        sut.specText = device.specText
        return SafeAreaViewController(safeAreaView: sut).view
    }
