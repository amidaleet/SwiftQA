...
final class SnapshotMatcherTests: XCTestCase {
    ...
    func test_SimpleFrames_PointMetal() async {
        await assertNoFalseErrorOnSimpleFrames(.pointColorDiffMetal)
    }
    ...
    func test_Text_PointMetal() async {
        await assertNoFalseErrorOnText(.pointColorDiffMetal)
    }
    ...
    func test_Gradient_PointMetal() async {
        await assertNoFalseErrorOnGradient(.pointColorDiffMetal)
    }
    ...
    func test_ButtonShiftOnePoint_PointConcurrent() async {
        await assertDetectsButtonShift(1, .pointColorDiffConcurrent, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    func test_ButtonShiftOnePoint_PointMetal() async {
        await assertDetectsButtonShift(1, .pointColorDiffMetal, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    // MARK: - Button Shift 10

    func test_ButtonShiftTenPoints_PointConcurrent() async {
        await assertDetectsButtonShift(10, .pointColorDiffConcurrent, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    func test_ButtonShiftTenPoints_PointMetal() async {
        await assertDetectsButtonShift(10, .pointColorDiffMetal, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    // MARK: - 1 Character color

    func test_CharacterColorChanged_PointConcurrent() async {
        await assertDetectsCharacterColorChange(.pointColorDiffConcurrent, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    func test_CharacterColorChanged_PointMetal() async {
        await assertDetectsCharacterColorChange(.pointColorDiffMetal, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    // MARK: - Button color

    func test_ButtonColorChanged_PointConcurrent() async {
        await assertDetectsButtonColorChange(.pointColorDiffConcurrent, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }

    func test_ButtonColorChanged_PointMetal() async {
        await assertDetectsButtonColorChange(.pointColorDiffMetal, expected: Array(repeating: SnapshotError.testFailed("/"), count: 6))
    }
